...
def extract_us_comm_data(pdf_path):
    """
    Extract US commodity data from the PDF file using multiple methods
    """
    all_tables = []
    all_text_data = []
...
def save_tables_to_csv(tables, base_filename="extracted_us_comm_table"):
    """
...
def save_text_data(text_data, filename="us_comm_text_data.json"):
    """
...
def main():
    ...
    try:
        ...
        tables, text_data = extract_us_comm_data(pdf_path)
        ...
        if tables:
            ...
            save_tables_to_csv(tables, "us_comm_table")
            
            # Also save raw table data as JSON for reference
            with open("us_comm_tables_raw.json", "w") as f:
                json.dump(tables, f, indent=2)
            print("Saved raw table data to us_comm_tables_raw.json")
        
        # Save text data for manual inspection
        if text_data:
            save_text_data(text_data, "us_comm_text_data.json")
            
        if not tables:
            print("\nNo structured tables were found.")
            print("Check us_comm_text_data.json for raw text that might contain tabular data.")
